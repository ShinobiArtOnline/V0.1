...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("Konoha is under attack, you must {help} sasuke with Gaara.", cid)
		addFocus(cid)

	elseif (isFocused(cid) and msg == "help" and getPlayerStorageValue(cid,8000) <= 14) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "help" and getPlayerStorageValue(cid,8000) >= 16) then
		selfSay("Sorry, but you can\'t do this {saga} any more.", cid)

	elseif (isFocused(cid) and msg == "help") then
		
		
		setPlayerStorageValue(cid,8000,16)
		selfSay("This is A Rank Mission!.", cid)
