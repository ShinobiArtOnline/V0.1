...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("Hah you think I will {go} with you?.", cid)
		addFocus(cid)
	elseif(isFocused(cid) and not isPremium(cid)) then
			selfSay("Only premium players can enter shippuuden saga.",cid)

	elseif (isFocused(cid) and msg == "go" and getPlayerStorageValue(cid,8000) <= 43) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "go" and getPlayerStorageValue(cid,8000) >= 45) then
		selfSay("Sorry, but you can\'t do this {saga} any more.", cid)

	elseif (isFocused(cid) and msg == "go") then
		
		
		setPlayerStorageValue(cid,8000,45)
		selfSay("Ok but first we will find the Zabuzas Sword.", cid)
