...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("That was very hard we can back to {Konoha}.", cid)
		addFocus(cid)

	elseif (isFocused(cid) and msg == "Konoha" and getPlayerStorageValue(cid,8000) <= 42) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "Konoha" and getPlayerStorageValue(cid,8000) >= 44) then
	...
	elseif (isFocused(cid) and msg == "Konoha") then
		...
		selfSay("Ok lets go to the village!.", cid)
