...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("So what is your {answer} about this.", cid)
		addFocus(cid)
	elseif(isFocused(cid) and not isPremium(cid)) then
			selfSay("Only premium players can enter shippuuden saga.",cid)

	elseif (isFocused(cid) and msg == "answer" and getPlayerStorageValue(cid,8000) <= 57) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "answer" and getPlayerStorageValue(cid,8000) >= 59) then
		selfSay("Sorry, but you can\'t do this {saga} any more.", cid)

	elseif (isFocused(cid) and msg == "answer") then
		
		
		setPlayerStorageValue(cid,8000,59)
		selfSay("So this is your answer, from now i belive in you.", cid)

	elseif((isFocused(cid)) and (msg == "bye" or msg == "goodbye" or msg == "cya")) then
		selfSay("I belive in you", cid, TRUE)
		closeShopWindow(cid)
		removeFocus(cid)
	end
end
