...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("Haha you think i am {spy} and sasoris technique is on me?.", cid)
		addFocus(cid)
	elseif(isFocused(cid) and not isPremium(cid)) then
			selfSay("Only premium players can enter shippuuden saga.",cid)

	elseif (isFocused(cid) and msg == "spy" and getPlayerStorageValue(cid,8000) <= 36) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "spy" and getPlayerStorageValue(cid,8000) >= 38) then
		selfSay("Sorry, but you can\'t do this {saga} any more.", cid)

	elseif (isFocused(cid) and msg == "spy") then
		
		
		setPlayerStorageValue(cid,8000,38)
		selfSay("Orochimaru Sama!.", cid)
