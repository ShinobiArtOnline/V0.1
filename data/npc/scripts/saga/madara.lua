...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("I know {true} about Itachi.", cid)
		addFocus(cid)
	elseif(isFocused(cid) and not isPremium(cid)) then
			selfSay("Only premium players can enter shippuuden saga.",cid)

	elseif (isFocused(cid) and msg == "true" and getPlayerStorageValue(cid,8000) <= 49) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "true" and getPlayerStorageValue(cid,8000) >= 51) then
		selfSay("Sorry, but you can\'t do this {saga} any more.", cid)

	elseif (isFocused(cid) and msg == "true") then
		
		
		setPlayerStorageValue(cid,8000,51)
		selfSay("Now you know the True, Your first mission is to bring me the Hachibi.", cid)
