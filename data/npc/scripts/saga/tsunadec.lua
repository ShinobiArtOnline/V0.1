...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("Gaara was abducted, you must {rescue} him.", cid)
		addFocus(cid)
	elseif(isFocused(cid) and not isPremium(cid)) then
			selfSay("Only premium players can enter shippuuden saga.",cid)		

	elseif (isFocused(cid) and msg == "rescue" and getPlayerStorageValue(cid,8000) <= 27) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "rescue" and getPlayerStorageValue(cid,8000) >= 29) then
		selfSay("Sorry, but you can\'t do this {saga} any more.", cid)

	elseif (isFocused(cid) and msg == "rescue") then
		
		
		setPlayerStorageValue(cid,8000,29)
		selfSay("Go and bring Gaara to his village!.", cid)
