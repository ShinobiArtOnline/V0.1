...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("Thank you, I think you should be {hokage}.", cid)
		addFocus(cid)

	elseif (isFocused(cid) and msg == "hokage" and getPlayerStorageValue(cid,8000) <= 35) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "hokage" and getPlayerStorageValue(cid,8000) >= 37) then
		selfSay("Sorry, but you can\'t do this {saga} any more.", cid)

	elseif (isFocused(cid) and msg == "hokage") then
		
		
		setPlayerStorageValue(cid,8000,37)
		selfSay("I know about Sasuke, Bye.", cid)
