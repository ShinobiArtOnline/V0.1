...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("Yo, ".. getCreatureName(cid) ..". we have a {chunin exam} in Konoha.", cid)
		addFocus(cid)

	elseif (isFocused(cid) and msg == "chunin exam" and getPlayerStorageValue(cid,8000) <= 6) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "chunin exam" and getPlayerStorageValue(cid,8000) >= 8) then
		selfSay("Sorry, but you can\'t do this {saga} any more.", cid)

	elseif (isFocused(cid) and msg == "chunin exam") then
		
		
		setPlayerStorageValue(cid,8000,8)
		selfSay("I excepted that, ".. getCreatureName(cid) ..". Go to this building and start the theory exam.", cid)
