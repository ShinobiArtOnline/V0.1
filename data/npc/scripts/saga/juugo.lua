...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("Ok I will go with you, you are like {Kimimaro}.", cid)
		addFocus(cid)

	elseif (isFocused(cid) and msg == "Kimimaro" and getPlayerStorageValue(cid,8000) <= 46) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "Kimimaro" and getPlayerStorageValue(cid,8000) >= 48) then
		selfSay("Sorry, but you can\'t do this {saga} any more.", cid)

	elseif (isFocused(cid) and msg == "Kimimaro") then
		
		
		setPlayerStorageValue(cid,8000,48)
		selfSay("You must control my power.", cid)
