...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("I dont have any {bonds} of you.", cid)
		addFocus(cid)

	elseif (isFocused(cid) and msg == "bonds" and getPlayerStorageValue(cid,8000) <= 38) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "bonds" and getPlayerStorageValue(cid,8000) >= 40) then
		selfSay("Sorry, but you can\'t do this {saga} any more.", cid)

	elseif (isFocused(cid) and msg == "bonds") then
		
		
		setPlayerStorageValue(cid,8000,40)
		selfSay("I am able to kill you now.", cid)
