...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("Ok, Us mission is to bring Sasuke to Konoha so lets {start}.", cid)
		addFocus(cid)

	elseif (isFocused(cid) and msg == "start" and getPlayerStorageValue(cid,8000) <= 20) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "start" and getPlayerStorageValue(cid,8000) >= 22) then
		selfSay("Sorry, but you can\'t do this {saga} any more.", cid)

	elseif (isFocused(cid) and msg == "start") then
		
		
		setPlayerStorageValue(cid,8000,23)
		selfSay("He is going to South", cid)
