...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("Yo, from now you are a hero, we must rebuild the {konoha}.", cid)
		addFocus(cid)

	elseif (isFocused(cid) and msg == "konoha" and getPlayerStorageValue(cid,8000) <= 58) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "konoha" and getPlayerStorageValue(cid,8000) >= 60) then
		selfSay("Sorry, but you can\'t do this {saga} any more.", cid)

	elseif (isFocused(cid) and msg == "konoha") then
		
		
		setPlayerStorageValue(cid,8000,60)
		selfSay("Good work, you are so close to be a Hokage.", cid)
