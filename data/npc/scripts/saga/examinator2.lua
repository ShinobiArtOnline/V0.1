...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("Ok you mast {loss} your final opponent", cid)
		addFocus(cid)

	elseif (isFocused(cid) and msg == "loss" and getPlayerStorageValue(cid,8000) <= 12) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "loss" and getPlayerStorageValue(cid,8000) >= 14) then
		selfSay("Sorry, but you can\'t do this {saga} any more.", cid)

	elseif (isFocused(cid) and msg == "loss") then
		
		
		setPlayerStorageValue(cid,8000,14)
		selfSay("Your Opponent is Neji!.", cid)
