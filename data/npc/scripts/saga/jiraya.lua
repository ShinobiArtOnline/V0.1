...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("Hey, Konoha no have any Hokage, we must bring {Tsunade}.", cid)
		addFocus(cid)

	elseif (isFocused(cid) and msg == "Tsunade" and getPlayerStorageValue(cid,8000) <= 16) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "Tsunade" and getPlayerStorageValue(cid,8000) >= 18) then
		selfSay("Sorry, but you can\'t do this {saga} any more.", cid)

	elseif (isFocused(cid) and msg == "Tsunade") then
		
		
		setPlayerStorageValue(cid,8000,18)
		selfSay("Yes, we must find Tsunade, I expect she is near Sand Village or some another small village.", cid)
