...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("Thank you for all Naruto, so you want me to be a Hokage right?.", cid)
		addFocus(cid)

	elseif (isFocused(cid) and msg == "yes" and getPlayerStorageValue(cid,8000) <= 19) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "yes" and getPlayerStorageValue(cid,8000) >= 21) then
	...
	elseif (isFocused(cid) and msg == "yes") then
		...
		selfSay("So Lets go to Konoha, oh and .. Take This.", cid)
