...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("Hey, ".. getCreatureName(cid) ..". We must escrot mr Tazuna to finish build his {bridge}.", cid)
		addFocus(cid)

	elseif (isFocused(cid) and msg == "bridge" and getPlayerStorageValue(cid,8000) <= 2) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "bridge" and getPlayerStorageValue(cid,8000) >= 4) then
		selfSay("Sorry, but you can\'t do this {saga} any more.", cid)

	elseif (isFocused(cid) and msg == "bridge") then
		doPlayerAddItem(cid,2525,1)
		
		setPlayerStorageValue(cid,8000,4)
		selfSay("Ok, ".. getCreatureName(cid) ..". Lets go!", cid)
