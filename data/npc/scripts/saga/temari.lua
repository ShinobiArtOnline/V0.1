...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("They Abducted Gaara to Hidden {Cave}.", cid)
		addFocus(cid)

	elseif (isFocused(cid) and msg == "cave" and getPlayerStorageValue(cid,8000) <= 30) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "cave" and getPlayerStorageValue(cid,8000) >= 32) then
		selfSay("Sorry, but you can\'t do this {saga} any more.", cid)

	elseif (isFocused(cid) and msg == "cave") then
		
		
		setPlayerStorageValue(cid,8000,32)
		selfSay("Please bring Gaara back!.", cid)
