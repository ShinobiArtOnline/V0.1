...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("Ok, ".. getCreatureName(cid) ..". are you ready?.", cid)
		addFocus(cid)

	elseif (isFocused(cid) and msg == "yes" and getPlayerStorageValue(cid,8000) <= 8) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "yes" and getPlayerStorageValue(cid,8000) >= 10) then
	...
	elseif (isFocused(cid) and msg == "yes") then
		...
		selfSay("From now The chunin Exam was started!.", cid)
