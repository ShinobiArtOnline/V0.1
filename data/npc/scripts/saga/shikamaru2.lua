...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("I wanna {revenge} we will attack they on west sky-forest.", cid)
		addFocus(cid)
	elseif(isFocused(cid) and not isPremium(cid)) then
			selfSay("Only premium players can enter shippuuden saga.",cid)

	elseif (isFocused(cid) and msg == "revenge" and getPlayerStorageValue(cid,8000) <= 39) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "revenge" and getPlayerStorageValue(cid,8000) >= 41) then
		selfSay("Sorry, but you can\'t do this {saga} any more.", cid)

	elseif (isFocused(cid) and msg == "revenge") then
		
		
		setPlayerStorageValue(cid,8000,41)
		selfSay("Ok lets go I am belive on you.", cid)
