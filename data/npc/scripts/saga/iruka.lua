...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("Welcome, so you wanna to be a {genin} right? ", cid)
		addFocus(cid)
        elseif (isFocused(cid) and msg == "genin" and getPlayerStorageValue(cid,8000) >= 1) then
		selfSay("Sorry, but you done thix exam.", cid)
	elseif (isFocused(cid) and msg == "genin") then
		setPlayerStorageValue(cid,8000,1)
		
	selfSay("Sorry, but you cant be a genin with that skills!!.", cid)
