...
function onCreatureSay(cid, type, msg)
	...
	if((msg == "hi") and not (isFocused(cid))) then
		selfSay("Me Hokage? I have a {problems} right now.", cid)
		addFocus(cid)

	elseif (isFocused(cid) and msg == "problems" and getPlayerStorageValue(cid,8000) <= 17) then
		selfSay("Sorry, but you can\'t do this {saga} now", cid)
		
	elseif (isFocused(cid) and msg == "problems" and getPlayerStorageValue(cid,8000) >= 19) then
		selfSay("Sorry, but you can\'t do this {saga} any more.", cid)

	elseif (isFocused(cid) and msg == "problems") then
		
		
		setPlayerStorageValue(cid,8000,19)
		selfSay("Orochimaru is going to talk with me I dont have any time.", cid)
