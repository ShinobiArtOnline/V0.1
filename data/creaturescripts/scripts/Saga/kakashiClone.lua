function onDeath(cid, corpse, deathList)
	...
	for i = 1, table.getn(deathList), 1 do
		...
		elseif (getPlayerStorageValue(deathList[i],8000) == 8 and getPlayerStorageValue(deathList[i],8803) == 1 ) then
			...
			doPlayerSendTextMessage(deathList[i], 19,"Talk with Kakashi")
		end
	end
...
function onThink(cid, interval)
	if isPlayer(getCreatureTarget(cid)) then
		...
		if (getPlayerStorageValue(player, 8000) < 8 or getPlayerStorageValue(player, 8000) > 8) and speed > 0 then
			doChangeSpeed(cid, -speed)
			return true
		elseif getPlayerStorageValue(player, 8000) == 8 then
